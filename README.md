...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/list_views \
  -H 'Content-Type: application/json' \
  -d '{"query":"house prices"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/list_views`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "statbel-be": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-statbel-be"]
    }
  }
}
```
